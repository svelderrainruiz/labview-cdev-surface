...
Describe 'Linux LabVIEW image gate workflow contract' {
    ...
    It 'defines resolve, linux prep, windows prerequisites, hosted linux ppl signal lane, linux parity, windows vip blocking lane, and summary lanes' {
        ...
        $script:coreWorkflowContent | Should -Match 'runs-on:\s*\[self-hosted,\s*windows,\s*self-hosted-windows-lv,\s*cdev-surface-windows-gate,\s*headless,\s*linux-gate-windows-ready\]'
        $script:coreWorkflowContent | Should -Match '(?ms)windows-host-ppl-32:\s*.*?runs-on:\s*\[self-hosted,\s*windows,\s*self-hosted-windows-lv,\s*cdev-surface-windows-gate,\s*headless,\s*linux-gate-windows-ready\]'
        $script:coreWorkflowContent | Should -Match '(?ms)windows-host-ppl-64:\s*.*?runs-on:\s*\[self-hosted,\s*windows,\s*self-hosted-windows-lv,\s*cdev-surface-windows-gate,\s*headless,\s*linux-gate-windows-ready\]'
        $script:coreWorkflowContent | Should -Match '(?ms)windows-host-vip-build:\s*.*?runs-on:\s*\[self-hosted,\s*windows,\s*self-hosted-windows-lv,\s*cdev-surface-windows-gate,\s*headless,\s*linux-gate-windows-ready\]'
        $script:coreWorkflowContent | Should -Not -Match '(?ms)windows-host-ppl-32:\s*.*?runs-on:\s*\[[^\]]*user-session'
