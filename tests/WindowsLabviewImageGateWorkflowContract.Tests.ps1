...
Describe 'Windows LabVIEW image gate workflow contract' {
    ...
    It 'enforces release-vs-parity post-action roles and vi analyzer parity execution' {
        ...
        $script:coreWorkflowContent | Should -Match 'Host release VIP install/uninstall check failed'
        $script:coreWorkflowContent | Should -Match 'vip-install-uninstall-check\.2026x64\.json'
        $script:coreWorkflowContent | Should -Match 'vip-install-uninstall-check\.2026x64\.log'
        $script:coreWorkflowContent | Should -Match 'vip_install_uninstall_status'
        $script:coreWorkflowContent | Should -Match 'vip_install_uninstall_report'
        $script:coreWorkflowContent | Should -Match 'Running vipm activate prior to install/uninstall verification'
        $script:coreWorkflowContent | Should -Match '''--labview-version'', ''2026'', ''--labview-bitness'', ''64'''
        $script:coreWorkflowContent | Should -Match '''uninstall'', \$uninstallTarget'
        ...
        $script:coreWorkflowContent | Should -Match 'git-safe-directories-report\.host-release\.json'
        $script:coreWorkflowContent | Should -Match 'git-safe-directories-report\.host-preflight\.json'
        $script:coreWorkflowContent | Should -Match 'git-safe-directories-report\.container-parity\.json'
    ...
    It 'keeps valid feed-add command ordering and container fallback diagnostics contract' {
        ...
        $script:coreWorkflowContent | Should -Match '\$requiredCommands = @\(''powershell'', ''git'', ''gh'', ''g-cli''\)'
        ...
        $script:coreWorkflowContent | Should -Match 'LABVIEW_WINDOWS_DOCKER_DNS'
        $script:coreWorkflowContent | Should -Match '''--dns'', \$dnsServer'
        $script:coreWorkflowContent | Should -Match 'if \(\$hasHostPwshRoot\) \{'
        $script:coreWorkflowContent | Should -Match 'Host PowerShell 7 root is not present'
        ...
        $script:coreWorkflowContent | Should -Not -Match 'target=C:\\Program Files\\National Instruments\\LabVIEW 2020,readonly'
        ...
        $script:coreWorkflowContent | Should -Match 'installer_timeout'
        $script:coreWorkflowContent | Should -Match 'vi_analyzer_timeout'
        $script:coreWorkflowContent | Should -Match 'postphase_copy_timeout'
        $script:coreWorkflowContent | Should -Match 'phase-checkpoints\.log'
        $script:coreWorkflowContent | Should -Match 'container-execution-classification\.json'
        $script:coreWorkflowContent | Should -Match 'windows_container_parity_heartbeat'
        $script:coreWorkflowContent | Should -Match 'container-fallback-diagnostics\.json'
        $script:coreWorkflowContent | Should -Match 'docker_dns_servers = @\(\$dockerDnsServers\)'
