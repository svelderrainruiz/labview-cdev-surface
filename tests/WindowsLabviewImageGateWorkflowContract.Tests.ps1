...
Describe 'Windows LabVIEW image gate workflow contract' {
    ...
    It 'targets windows containers with installer-post-action report checks in core workflow' {
        ...
        $script:coreWorkflowContent | Should -Match 'LVIE_GATE_REQUIRED_LABVIEW_YEAR=\$gateRequiredLabviewYear'
        $script:coreWorkflowContent | Should -Match 'LVIE_GATE_SINGLE_PPL_BITNESS=\$singlePplBitness'
        $script:coreWorkflowContent | Should -Match '\$labviewX86NipkgInstallCmdForContainer = \$labviewX86NipkgInstallCmd'
        $script:coreWorkflowContent | Should -Match 'x86-nipkg-bootstrap\.cmd'
        $script:coreWorkflowContent | Should -Match 'Set-Content -LiteralPath \$x86NipkgCmdScriptPath -Value \$x86NipkgCmdScriptContent -Encoding ascii'
        $script:coreWorkflowContent | Should -Match 'LVIE_LABVIEW_X86_NIPKG_INSTALL_CMD=\$labviewX86NipkgInstallCmdForContainer'
        ...
        $script:coreWorkflowContent | Should -Match 'type=bind,source=\$hostDevRoot,target=C:\\dev'
        $script:coreWorkflowContent | Should -Match 'type=bind,source=\$hostLabview2020x64Root,target=C:\\Program Files\\National Instruments\\LabVIEW 2020,readonly'
        $script:coreWorkflowContent | Should -Match 'type=bind,source=\$hostLabview2020x86Root,target=C:\\Program Files \(x86\)\\National Instruments\\LabVIEW 2020,readonly'
        $script:coreWorkflowContent | Should -Match 'type=bind,source=\$hostGitRoot,target=C:\\host-tools\\Git,readonly'
        $script:coreWorkflowContent | Should -Match 'type=bind,source=\$hostGhRoot,target=C:\\host-tools\\GitHubCLI,readonly'
        $script:coreWorkflowContent | Should -Match 'type=bind,source=\$hostGCliRoot,target=C:\\host-tools\\G-CLI,readonly'
        $script:coreWorkflowContent | Should -Match '\$containerCmdEncoded = \[Convert\]::ToBase64String\(\[System\.Text\.Encoding\]::Unicode\.GetBytes\(\$containerCmd\)\)'
        $script:coreWorkflowContent | Should -Match '-EncodedCommand'
        $script:coreWorkflowContent | Should -Match '\$containerCmdEncoded'
        $script:coreWorkflowContent | Should -Match '& docker @dockerArgs'
